...
from logica.determinantes import determinante, DeterminantesError, get_matrix_dims
...
class CalculadoraAlgebraLineal:
   ...
   # ... (Dentro de la clase CalculadoraAlgebraLineal)
    ...
    def crear_pestaña_determinantes(self):
        """Crea la pestaña para el Cálculo de Determinantes."""
        det_frame = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(det_frame, text="Determinante") # Nombre de pestaña simple
        
        # 1. Configuración de Matriz
        config_frame = ttk.Frame(det_frame)
        config_frame.grid(row=0, column=0, sticky='ew', pady=(0, 10))
        config_frame.columnconfigure(1, weight=1)
        
        ttk.Label(config_frame, text="Dimensión N x N:").grid(row=0, column=0, padx=5, sticky='w')
        ...
        spinbox_dim.grid(row=0, column=1, padx=(5, 10), sticky='w')
        
        ttk.Button(config_frame, text="Generar Matriz", 
                   command=self.det_actualizar_grid, 
                   style='Accent.TButton').grid(row=0, column=2, padx=10)

        # 2. Marco para la Matriz
        self.det_matriz_frame = ttk.Frame(det_frame, padding="10")
        self.det_matriz_frame.grid(row=1, column=0, sticky='nsew')
        self.det_matriz_frame.grid_columnconfigure(0, weight=1)
        self.det_matriz_frame.grid_rowconfigure(0, weight=1)

        # 3. Marco para Resultados y Botones
        det_output_frame = ttk.Frame(det_frame, padding="10")
        det_output_frame.grid(row=2, column=0, sticky='ew', pady=(10, 0))
        det_output_frame.columnconfigure(0, weight=1)

        ttk.Button(det_output_frame, text="Calcular Determinante |A|", 
                   command=self.calcular_determinante).grid(row=0, column=0, padx=10, sticky='ew')
                   
        self.det_results_label = ttk.Label(det_output_frame, text="Resultado: -", font=font.Font(size=14, weight='bold'))
        self.det_results_label.grid(row=1, column=0, columnspan=2, pady=(15, 0), sticky='w')
    ...
    def det_obtener_matriz(self):
        ...
        for i, row_entries in enumerate(self.det_matrix_entries):
            ...
            for j, entry in enumerate(row_entries):
                try:
                    # Permite comas como separador decimal
                    value = float(entry.get().replace(",", "."))
                    row_float.append(value)
                except ValueError:
                    raise ValueError(f"Valor no válido en la celda ({i+1}, {j+1}): '{entry.get()}'")
    ...
    def calcular_determinante(self):
        """Llama a la lógica del determinante y muestra el resultado."""
