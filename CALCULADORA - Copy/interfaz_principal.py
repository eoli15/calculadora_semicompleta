...
from logica.determinantes import determinante, DeterminantesError, get_matrix_dims, determinante_con_pasos, matrix_to_string
from logica.cramer import resolver_por_cramer
...
class CalculadoraAlgebraLineal:
    ...
    def configurar_interfaz(self):
        ...
        self.crear_pestaña_determinantes()
        self.crear_pestaña_cramer()
   ...
   # ... (Dentro de la clase CalculadoraAlgebraLineal)

    # En interfaz_principal.py

    def crear_pestaña_determinantes(self):
        """Crea la pestaña para el Cálculo de Determinantes, con los pasos debajo."""
        self.det_frame = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(self.det_frame, text="Determinante") 
        
        # Configurar el grid: [0] es la matriz/controles, [1] es el área de pasos
        self.det_frame.grid_rowconfigure(2, weight=1) # El área de texto ocupa el espacio
        self.det_frame.grid_columnconfigure(0, weight=1)

        # ----------------------------------------------------
        # 1. Marco Superior (Configuración, Matriz, Resultados)
        # ----------------------------------------------------
        top_frame = ttk.Frame(self.det_frame)
        top_frame.grid(row=0, column=0, sticky='ew')
        top_frame.columnconfigure(0, weight=1) 

        # 1.1. Marco para la Matriz (Izquierda)
        self.det_matriz_frame = ttk.Frame(top_frame, padding="10")
        self.det_matriz_frame.grid(row=0, column=0, sticky='nsew')
        
        # 1.2. Marco para Configuración y Resultados (Derecha)
        control_output_frame = ttk.Frame(top_frame, padding="10")
        control_output_frame.grid(row=0, column=1, sticky='nsw')

        # Configuración de Dimensión (dentro de control_output_frame)
        config_frame = ttk.Frame(control_output_frame)
        config_frame.pack(fill='x', pady=(0, 10))
        
        ttk.Label(config_frame, text="N x N:").pack(side=tk.LEFT, padx=5)
        self.det_dim_var = tk.StringVar(value="3")
        spinbox_dim = ttk.Spinbox(config_frame, from_=2, to=8, increment=1, 
                                 textvariable=self.det_dim_var, width=5, 
                                 command=self.det_actualizar_grid)
        spinbox_dim.pack(side=tk.LEFT, padx=(5, 10))
        
        ttk.Button(config_frame, text="Generar", 
                   command=self.det_actualizar_grid, 
                   style='Accent.TButton').pack(side=tk.LEFT, padx=10)

        # Botón de Cálculo y Resultado
        ttk.Button(control_output_frame, text="Calcular Determinante |A|", 
                   command=self.calcular_determinante, 
                   style='Accent.TButton').pack(fill='x', pady=(10, 20))
                   
        self.det_results_label = ttk.Label(control_output_frame, text="Resultado: -", font=font.Font(size=14, weight='bold'))
        self.det_results_label.pack(fill='x')


        # ----------------------------------------------------
        # 2. Marco Inferior (Pasos)
        # ----------------------------------------------------
        ttk.Label(self.det_frame, text="Pasos Detallados de la Reducción Gaussiana:").grid(row=1, column=0, sticky='w', pady=(10, 5))
        
        self.det_pasos_text = scrolledtext.ScrolledText(self.det_frame, wrap=tk.WORD, 
                                                        font=("Consolas", 10), 
                                                        height=15)
        self.det_pasos_text.grid(row=2, column=0, sticky='nsew')
        self.det_pasos_text.insert(tk.END, "El proceso paso a paso aparecerá aquí tras el cálculo.")
        self.det_pasos_text.config(state=tk.DISABLED) # Inicia deshabilitado
        
        # Inicializar la matriz
        self.det_matrix_entries = []
        self.det_actualizar_grid()


    def det_actualizar_grid(self):
        """Actualiza la cuadrícula de entrada para la pestaña de determinantes."""
        try:
            N = int(self.det_dim_var.get())
        except ValueError:
            N = 3
            self.det_dim_var.set("3")
        
        if N < 2 or N > 8:
             messagebox.showwarning("Advertencia", "La dimensión debe estar entre 2 y 8.")
             N = 3
             self.det_dim_var.set("3")

        # Limpiar el frame anterior
        for widget in self.det_matriz_frame.winfo_children():
            widget.destroy()
        ...
        for i in range(N):
            row_entries = []
            for j in range(N):
                entry = ttk.Entry(self.det_matriz_frame, width='1', justify='center')
                entry.grid(row=i, column=j, padx=2, pady=2, sticky='nsew')
                entry.insert(0, "0" if i != j else "1") # Inicializa como matriz identidad
                row_entries.append(entry)
            self.det_matrix_entries.append(row_entries)

        # Configurar pesos para las celdas
        for i in range(N):
            self.det_matriz_frame.grid_rowconfigure(i, weight=1)
            self.det_matriz_frame.grid_columnconfigure(i, weight=1)
            
        self.det_results_label.config(text="Resultado: -")


    def det_obtener_matriz(self):
        """Extrae los valores de la matriz N x N de la pestaña de determinantes, 
        aceptando números, decimales o fracciones."""
        matrix_float = []
        for i, row_entries in enumerate(self.det_matrix_entries):
            row_float = []
            for j, entry in enumerate(row_entries):
                value_str = entry.get().strip()
                
                # Permite comas como separador decimal si no es una fracción
                if '/' not in value_str:
                    value_str = value_str.replace(",", ".")
                
                try:
                    # 1. Intentar manejar como FRACCIÓN (ej: 1/2)
                    if '/' in value_str and value_str.count('/') == 1:
                        numerator_str, denominator_str = value_str.split('/')
                        ...
                        # 2. Intentar manejar como NÚMERO ESTÁNDAR (entero o decimal)
                        value = float(value_str)
                    
                    row_float.append(value)
                    
                except ValueError as e:
                    # Captura errores de float() o errores de división por cero
                    error_msg = str(e) if "denominador" in str(e) else "Valor no válido (debe ser número, decimal o fracción)."
                    raise ValueError(f"Error en la celda ({i+1}, {j+1}): '{entry.get()}' -> {error_msg}")
                    
            matrix_float.append(row_float)
    ...
    def calcular_determinante(self):
        """Llama a la lógica del determinante, muestra el resultado y los pasos."""
        self.status_bar.config(text="Calculando Determinante...")
        try:
            matrix_A = self.det_obtener_matriz()
            
            N = len(matrix_A)
            if N == 0 or N != len(matrix_A[0]):
                raise DeterminantesError("La matriz no es cuadrada (N x N).")
                
            # >>> LLAMAR A LA FUNCIÓN QUE DEVUELVE LOS PASOS
            det_val, pasos_detalle = determinante_con_pasos(matrix_A)
            
            # 3. Mostrar Resultado Final
            det_val_str = f"{det_val:.6f}" if abs(det_val - round(det_val)) > 1e-8 else str(round(det_val))
            
            self.det_results_label.config(text=f"Resultado: det(A) = {det_val_str}")
            
            # 4. Mostrar pasos en el widget ScrolledText
            self.mostrar_pasos_determinante(pasos_detalle, det_val_str)
            # 4. Guardar datos para exportación
            self.ultimo_tipo_calculo = f"Determinante {get_matrix_dims(matrix_A)}"
            self.ultimos_datos_entrada = matrix_A
            self.ultimos_pasos = pasos_detalle # Usar la lista de pasos para exportación
            self.ultimo_resultado = f"det(A) = {det_val_str}"
            
            # >>> MOSTRAR LA PESTAÑA DE PASOS
        
            messagebox.showinfo("Éxito", f"Determinante calculado: {det_val_str}")
            self.status_bar.config(text="Determinante calculado con éxito.")
            
        # ... (resto del try-except)
            
        except ValueError as e:
            messagebox.showerror("Error de Entrada", f"Por favor, revisa la entrada: {str(e)}")
            self.status_bar.config(text="Error de entrada de datos.")
        except DeterminantesError as e:
    ...
    def crear_pestana_pasos_determinante(self):
        """Crea la pestaña para mostrar los pasos detallados del cálculo del determinante."""
        self.det_pasos_frame = ttk.Frame(self.notebook, padding="10")
        self.notebook.add(self.det_pasos_frame, text="Pasos Detallados (Det)")
        
        self.det_pasos_text = scrolledtext.ScrolledText(self.det_pasos_frame, wrap=tk.WORD, 
                                                        font=("Consolas", 10), 
                                                        height=15)
        self.det_pasos_text.pack(fill=tk.BOTH, expand=True)
        self.det_pasos_text.insert(tk.END, "Presiona 'Calcular Determinante' para ver los pasos aquí.")

        # Botón para volver a la pestaña de cálculo
        btn_back = ttk.Button(self.det_pasos_frame, text="Volver a Cálculo", 
                              command=lambda: self.notebook.select(self.det_frame)) # Asume self.det_frame existe
        btn_back.pack(pady=10)


    def mostrar_pasos_determinante(self, pasos_detalle, det_val_str):
        """Formatea y muestra los pasos del determinante en el área de texto de la misma pestaña."""
        
        self.det_pasos_text.config(state=tk.NORMAL)
        self.det_pasos_text.delete(1.0, tk.END)
        
        if not pasos_detalle:
            self.det_pasos_text.insert(tk.END, "No hay pasos detallados disponibles.")
            self.det_pasos_text.config(state=tk.DISABLED)
            return

        self.det_pasos_text.insert(tk.END, "=" * 80 + "\n")
        self.det_pasos_text.insert(tk.END, "PROCESO DE CÁLCULO DE DETERMINANTE (REDUCCIÓN GAUSSIANA)\n")
        self.det_pasos_text.insert(tk.END, "=" * 80 + "\n\n")

        for i, paso in enumerate(pasos_detalle):
            self.det_pasos_text.insert(tk.END, f"PASO {i}: {paso.operacion}\n")
            
            # Usar la función matrix_to_string del módulo de lógica
            matriz_str = matrix_to_string(paso.matriz_resultado)
            for line in matriz_str.split('\n'):
                self.det_pasos_text.insert(tk.END, f"  {line}\n")
            
            signo = "-" if paso.row_swaps % 2 != 0 else "+"
            self.det_pasos_text.insert(tk.END, f"  (Intercambios: {paso.row_swaps}. Factor signo: {signo})\n")
            self.det_pasos_text.insert(tk.END, "\n")
            
        
        self.det_pasos_text.insert(tk.END, "=" * 80 + "\n")
        self.det_pasos_text.insert(tk.END, f"RESULTADO FINAL (con ajustes de signo): {det_val_str}\n")
        self.det_pasos_text.insert(tk.END, "=" * 80 + "\n")
        
        self.det_pasos_text.config(state=tk.DISABLED)
        self.det_pasos_text.see(tk.END) # Desplazar hasta el final
    ...
    def crear_pestaña_cramer(self):
        """Crea la pestaña para resolver sistemas con Regla de Cramer."""
        frame = ttk.Frame(self.notebook, padding=10)
        self.notebook.add(frame, text="Resolver (Cramer)")
        ...
        # Cramer SÓLO funciona para matrices cuadradas (n x n)
        ttk.Label(dim_frame, text="Dimensión (n×n):", font=self.fonts["normal"]).pack(side="left")
        self.cramer_dimension = tk.Spinbox(dim_frame, from_=2, to=8, width=5, value=3, 
                                       font=self.fonts["normal"], bg=self.colors["bg_light"], 
                                       buttonbackground=self.colors["primary"])
        self.cramer_dimension.pack(side="left", padx=5)
        
        ttk.Button(dim_frame, text="Generar Campos", 
                  command=self.generar_campos_cramer).pack(side="left", padx=15)
        
        entrada_frame = ttk.Frame(frame)
        entrada_frame.pack(fill="both", expand=True, padx=5, pady=10)
        
        self.cramer_matriz_frame = ttk.LabelFrame(entrada_frame, text="Matriz A (coeficientes)", padding=8)
        self.cramer_matriz_frame.pack(side="left", fill="both", expand=True, padx=5)
        
        self.cramer_vector_frame = ttk.LabelFrame(entrada_frame, text="Vector b (términos independientes)", padding=8)
        self.cramer_vector_frame.pack(side="right", fill="y", padx=5)
        
        botones_frame = ttk.Frame(frame, padding=5)
        botones_frame.pack(fill="x", padx=5, pady=10)
        
        # ESTE ES EL BOTÓN QUE LLAMA A LA FUNCIÓN QUE PREGUNTAS
        ttk.Button(botones_frame, text="Calcular con Cramer", style="Action.TButton",
                  command=self.resolver_cramer).pack(side="left", padx=5)
        
        ttk.Button(botones_frame, text="Limpiar", 
                  command=self.limpiar_cramer).pack(side="left", padx=5)
        ttk.Button(botones_frame, text="Guardar Resultado", 
                  command=self.guardar_resultado).pack(side="left", padx=5)
        ...
        self.cramer_resultado = scrolledtext.ScrolledText(resultado_frame, height=15, width=80, 
                                                   font=self.fonts["normal"], bg=self.colors["bg_light"], 
                                                   fg=self.colors["text_dark"], padx=8, pady=8)
        self.cramer_resultado.pack(fill="both", expand=True, padx=5, pady=5)
        
        # Generar campos iniciales
        self.generar_campos_cramer()

    def resolver_cramer(self):
        """Resuelve el sistema usando la Regla de Cramer."""
        try:
            # 1. Obtener datos de la interfaz
            A = self.validar_matriz(self.cramer_entries_matriz)
            if A is None:
                return
            
            b = self.validar_vector(self.cramer_entries_vector)
            if b is None:
                return
            
            self.status_bar.config(text="Resolviendo sistema con Regla de Cramer...")
            self.root.update()
            
            # 2. Llamar a la función lógica de cramer.py
            # (Aquí usamos la función que importaste)
            solucion = resolver_por_cramer(A, b)
            
            # 3. Mostrar resultado en la interfaz
            if solucion is not None:
                # Formatear el resultado para mostrarlo
                resultado_texto = f"=== SOLUCIÓN (Regla de Cramer) ===\n\nEl sistema tiene una solución única:\n\n"
                resultado_texto += "\n".join([f"  x{i+1} = {val:.4f}" for i, val in enumerate(solucion)])
                
                pasos = [resultado_texto] # Cramer no suele mostrar "pasos", solo la solución
                self.mostrar_resultado(self.cramer_resultado, pasos)
                
                # Guardar para exportación
                self.ultimo_resultado = solucion
                self.ultimos_pasos = pasos
                self.ultimo_tipo_calculo = "Regla de Cramer"
                self.status_bar.config(text="Sistema resuelto con Regla de Cramer")
            
            else:
                # El determinante era 0
                pasos = ["=== ANÁLISIS ===\n\nEl determinante de la matriz principal es 0.\nEl sistema no tiene solución única (puede tener infinitas soluciones o ninguna).\nLa Regla de Cramer no es aplicable."]
                self.mostrar_resultado(self.cramer_resultado, pasos)
                
                # Guardar para exportación
                self.ultimo_resultado = None
                self.ultimos_pasos = pasos
                self.ultimo_tipo_calculo = "Regla de Cramer (Sin Solución)"
                self.status_bar.config(text="Error: El sistema no tiene solución única")
            
            self.ultimos_datos_entrada = {"Matriz A": A, "Vector b": b}

        except Exception as e:
            messagebox.showerror("Error", f"Error al resolver con Cramer: {str(e)}")
            self.status_bar.config(text="Error en el cálculo")

    def generar_campos_cramer(self):
        """Genera los campos de entrada para el sistema Cramer (n x n)."""
        # Limpiar frames
        for widget in self.cramer_matriz_frame.winfo_children():
            widget.destroy()
        for widget in self.cramer_vector_frame.winfo_children():
            widget.destroy()
        
        n = int(self.cramer_dimension.get()) # n x n
        
        # Campos para matriz A
        self.cramer_entries_matriz = []
        for i in range(n):
            fila_entries = []
            for j in range(n):
                entry = tk.Entry(self.cramer_matriz_frame, width=8, font=self.fonts["normal"], 
                                 bg=self.colors["bg_light"], relief="solid", borderwidth=1)
                entry.grid(row=i, column=j, padx=4, pady=4, ipadx=3, ipady=3)
                fila_entries.append(entry)
            self.cramer_entries_matriz.append(fila_entries)
        
        # Campos para vector b
        self.cramer_entries_vector = []
        for i in range(n):
            entry = tk.Entry(self.cramer_vector_frame, width=10, font=self.fonts["normal"], 
                             bg=self.colors["bg_light"], relief="solid", borderwidth=1)
            entry.grid(row=i, column=0, padx=4, pady=4, ipadx=3, ipady=3)
            self.cramer_entries_vector.append(entry)

    def limpiar_cramer(self):
        """Limpia los campos de Cramer."""
        for fila in self.cramer_entries_matriz:
            for entry in fila:
                entry.delete(0, tk.END)
        for entry in self.cramer_entries_vector:
            entry.delete(0, tk.END)
        self.cramer_resultado.delete(1.0, tk.END)
